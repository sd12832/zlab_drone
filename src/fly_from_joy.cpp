...
void twist_callback(const geometry_msgs::Twist& twist_msg_in) {
    ...
    // Get Direction from wherever and relay it on to the drone
    twist_msg_out = twist_msg_in; 
}

void tak_land_callback(const zlab_drone::Tak_land& tak_land_in) {

    //Detect a Takeoff or Landing Instruction
    takeoff = tak_land_in.takeoff;
    landing = tak_land_in.landing; 
    move = tak_land_in.move;
    new_msg = 1;
